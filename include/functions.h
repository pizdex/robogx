#ifndef GUARD_FUNCTIONS_H
#define GUARD_FUNCTIONS_H

// Place all external function declarations in this file

extern void sub_080000F0(void);
extern void sub_080002C4(void *src, void *dest, u32 size);
extern void sub_08000498(void *arg0, u32 arg1, u32 arg2);
extern void sub_08000540(void);

// src/code_08000548.c

extern void sub_08000548(void);
extern void sub_08000588(void);
extern void sub_080005B4(void);
extern void sub_08000738(void);
extern u32 sub_08000750(void *buffer, u32 startAddr, u32 numBytes, u32 useLock);
extern u32 sub_080007A8(void *buffer, u32 startAddr, u32 numBytes, u32 useLock);
extern u32 sub_08000800(void *arg0, u32 eepromOffset, u32 fillValue, u32 arg3);
extern void sub_08000990(void);
extern void sub_08000A34(void);
extern void sub_08000A5C(void);
extern void sub_08000C90(u32 arg0);

// asm/code_08000AF4.s

// extern ? sub_08000EBC(?);
extern void sub_08000EE0(u16 *dst, u32 *src, u32 offset);
// extern ? sub_08000F78(?);
extern void sub_08000FEC(u16 *arg0, u32 arg1);
// extern ? sub_08001004(?);
// extern ? sub_0800101C(?);
// extern ? sub_08001068(?);
// extern ? sub_08001090(?);
// extern ? sub_080010C4(?);
// extern ? sub_08001138(?);
// extern ? sub_08001160(?);
extern void sub_080012B8(void);
// extern ? sub_0800134C(?);
// extern ? sub_080013A8(?);
// extern ? sub_080013F8(?);
extern void sub_0800142C(void);
extern void sub_08001A18(void);
// extern ? sub_08001A1A(?);
// extern ? sub_08001A20(?);
// extern ? sub_08001A28(?);
// extern ? sub_08001AE0(?);
// extern ? sub_08001BD0(?);
// extern ? sub_08001C60(?);
// extern ? sub_08001D40(?);
// extern ? sub_08001E6C(?);
// extern ? sub_08001E98(?);
// extern ? sub_08001ED8(?);
// extern ? sub_08001F84(?);
// extern ? sub_08001FB4(?);
// extern ? sub_08001FF4(?);
// extern ? sub_080020A4(?);
// extern ? sub_08002154(?);
// extern ? sub_080021D8(?);
// extern ? sub_080022C0(?);
// extern ? sub_080022F0(?);
// extern ? sub_08002340(?);
extern void sub_080023D4(void);
// extern ? sub_080023E4(?);
// extern ? sub_08002468(?);
// extern ? sub_08002806(?);
// extern ? sub_0800280C(?);
// extern ? sub_08002824(?);
// extern ? sub_08002844(?);
// extern ? sub_08002874(?);
// extern ? sub_0800288C(?);
// extern ? sub_0800288E(?);
// extern ? sub_080028A8(?);
// extern ? sub_080028AA(?);
// extern ? sub_080028B4(?);
// extern ? sub_080028D4(?);
// extern ? sub_080028F0(?);
// extern ? sub_08002904(?);
// extern ? sub_08002934(?);
// extern ? sub_08002940(?);
// extern ? sub_08002954(?);
// extern ? sub_08002968(?);
// extern ? sub_08002998(?);
// extern ? sub_080029AC(?);
// extern ? sub_080029C0(?);
// extern ? sub_080029D4(?);
// extern ? sub_080029E8(?);
// extern ? sub_080029F4(?);
// extern ? sub_08002A0C(?);
// extern ? sub_08002A20(?);
extern void sub_08002A38(void);
// extern ? sub_08002A84(?);
// extern ? sub_08002CDC(?);
// extern ? sub_08002CEC(?);
// extern ? sub_08002D30(?);
// extern ? sub_08002D60(?);
// extern ? sub_08002F60(?);
// extern ? sub_08002FA0(?);
// extern ? sub_08002FBC(?);
// extern ? sub_08002FC8(?);
// extern ? sub_08002FDC(?);
// extern ? sub_08002FF0(?);
// extern ? sub_08003058(?);
// extern ? sub_08003074(?);
extern void sub_08003094(void);
extern void sub_0800310C(void);
extern void sub_08003118(u32 arg0);
// extern ? sub_08003144(?);
// extern ? sub_08003190(?);
extern void sub_080031E4(u32 arg0);
// extern ? sub_08003218(?);
extern void sub_0800324C(void);
extern void sub_08003278(struct Unk08045158 *arg0);
// extern ? sub_08003284(?);
extern void sub_080032B0(struct Unk08045158 *arg0, u16 arg1);
// extern ? sub_080032C0(?);
// extern ? sub_080032E0(?);
// extern ? sub_08003308(?);
// extern ? sub_08003350(?);
// extern ? sub_0800346C(?);
// extern ? sub_08003480(?);
// extern ? sub_08003494(?);
// extern ? sub_0800358C(?);
extern void sub_08003630(u32 arg0);
// extern ? sub_080036C8(?);
extern void sub_0800371C(void);
extern void sub_08003798(void);
// extern ? sub_080037D4(?);
extern void sub_0800384C(struct Unk08045158 *arg0, u32 arg1); // Probably wrong
extern void sub_08003930(struct Unk08045158 *arg0);
// extern ? sub_08003970(?);
// extern ? sub_08003A38(?);
// extern ? sub_08003B94(?);
// extern ? sub_08003BE4(?);
// extern ? sub_08003C4C(?);
extern void sub_080040C0(struct Unk08045158 *arg0, u32 arg1, u16 arg2);
// extern ? sub_08004128(?);
// extern ? sub_0800419C(?);
// extern ? sub_08004204(?);
// extern ? sub_08004224(?);
// extern ? sub_08004298(?);
// extern ? sub_0800430C(?);
// extern ? sub_08004464(?);
// extern ? sub_08004484(?);
// extern ? sub_08004498(?);
// extern ? sub_080044E0(?);
// extern ? sub_080044F4(?);
// extern ? sub_08004508(?);
// extern ? sub_0800451C(?);
// extern ? sub_08004530(?);
// extern ? sub_08004544(?);
// extern ? sub_08004550(?);
// extern ? sub_0800455C(?);
// extern ? sub_08004570(?);
// extern ? sub_08004584(?);
// extern ? sub_08004588(?);
// extern ? sub_08004594(?);
// extern ? sub_0800459C(?);
// extern ? sub_08004604(?);
// extern ? sub_0800469C(?);
// extern ? sub_080046B8(?);
// extern ? sub_080046D4(?);
// extern ? sub_080046F0(?);
// extern ? sub_08004714(?);
// extern ? sub_08004788(?);
// extern ? sub_080047F8(?);
// extern ? sub_08004824(?);
// extern ? sub_08004918(?);
// extern ? sub_08004954(?);
// extern ? sub_08004990(?);
// extern ? sub_08004A2C(?);
// extern ? sub_08004AA8(?);
// extern ? sub_08004B1C(?);
// extern ? sub_08004B58(?);
// extern ? sub_08004B98(?);
// extern ? sub_08004CAC(?);
// extern ? sub_08004CF0(?);
// extern ? sub_08004D0C(?);
// extern ? sub_08004D2C(?);
// extern ? sub_08004D5C(?);
// extern ? sub_08004E28(?);
// extern ? sub_08004E6C(?);
// extern ? sub_08004EC0(?);
// extern ? sub_08004F68(?);
// extern ? sub_08005028(?);
extern void sub_080050A8(const char *str);
extern void sub_080050B8(const char *str);
// extern ? sub_080050C4(?);
// extern ? sub_080050E4(?);
// extern ? sub_0800510C(?);
// extern ? sub_0800512C(?);
// extern ? sub_08005148(?);
// extern ? sub_08005170(?);
// extern ? sub_08005198(?);
extern void sub_080051B0(u32 arg0);
// extern ? sub_080051F4(?);
// extern ? sub_08005214(?);
// extern ? sub_08005234(?);
// extern ? sub_080052AC(?);
// extern ? sub_080052CC(?);
// extern ? sub_080052D8(?);
// extern ? sub_08005398(?);
// extern ? sub_080053D8(?);
// extern ? sub_08005454(?);
// extern ? sub_080054D8(?);
// extern ? sub_080055A4(?);
// extern ? sub_08005674(?);
// extern ? sub_080056D8(?);
// extern ? sub_08005738(?);
// extern ? sub_0800579C(?);
// extern ? sub_080057C4(?);
// extern ? sub_0800591C(?);
// extern ? sub_08005964(?);
// extern ? sub_080059AC(?);
// extern ? sub_080059C4(?);
// extern ? sub_08005A0C(?);
// extern ? sub_08005A34(?);
// extern ? sub_08005A50(?);
// extern ? sub_08005A64(?);
// extern ? sub_08005AC0(?);
// extern ? sub_08005AFC(?);
// extern ? sub_08005B50(?);
// extern ? sub_08005C64(?);
// extern ? sub_08005C74(?);
// extern ? sub_08005CAC(?);
// extern ? sub_08005DB0(?);
// extern ? sub_08005E18(?);
// extern ? sub_08005E9C(?);
// extern ? sub_08005F9C(?);
// extern ? sub_08005FF8(?);
// extern ? sub_08006068(?);
// extern ? sub_08006088(?);
// extern ? sub_08006134(?);
// extern ? sub_08006160(?);
// extern ? sub_08006198(?);
// extern ? sub_080061B4(?);
// extern ? sub_0800620C(?);
// extern ? sub_0800629C(?);
// extern ? sub_0800630C(?);
// extern ? sub_080063C8(?);
// extern ? sub_08006624(?);
// extern ? sub_0800663C(?);
// extern ? sub_080066A4(?);
// extern ? sub_080066B8(?);
// extern ? sub_080066D4(?);
// extern ? sub_08006768(?);
// extern ? sub_080067EC(?);
// extern ? sub_080069E0(?);
// extern ? sub_080069E4(?);
// extern ? sub_080069E8(?);
// extern ? sub_080069EC(?);
// extern ? sub_080069F0(?);
// extern ? sub_080069F4(?);
// extern ? sub_080069F8(?);
// extern ? sub_08006A1C(?);
// extern ? sub_08006A2C(?);
// extern ? sub_08006A3C(?);
// extern ? sub_08006A4C(?);
// extern ? sub_08006A5C(?);
// extern ? sub_08006A6C(?);
// extern ? sub_08006A7C(?);
// extern ? sub_08006AC0(?);
// extern ? sub_08006ACC(?);
// extern ? sub_08006AD8(?);
// extern ? sub_08006B04(?);
// extern ? sub_08006B30(?);
// extern ? sub_08006B68(?);
// extern ? sub_08006B78(?);
// extern ? sub_08006BA8(?);
// extern ? sub_08006BE0(?);
// extern ? sub_08006C4C(?);
// extern ? sub_08006C64(?);
// extern ? sub_08006CD4(?);
// extern ? sub_08006D0C(?);
// extern ? sub_08006D44(?);
// extern ? sub_08006D7C(?);
// extern ? sub_08006E8C(?);
// extern ? sub_08006F94(?);
// extern ? sub_0800709C(?);
// extern ? sub_08007164(?);
// extern ? sub_0800719C(?);
// extern ? sub_080071A8(?);
// extern ? sub_080071B4(?);
// extern ? sub_08007230(?);
// extern ? sub_08007264(?);
// extern ? sub_080073A4(?);
// extern ? sub_080073F0(?);
// extern ? sub_080074A0(?);
// extern ? sub_08007574(?);
// extern ? sub_080075D8(?);
// extern ? sub_08007648(?);
// extern ? sub_08007668(?);
// extern ? sub_08007794(?);
// extern ? sub_08007924(?);
// extern ? sub_08007980(?);
// extern ? sub_08007A38(?);
// extern ? sub_08007A90(?);
// extern ? sub_08007AD4(?);
// extern ? sub_08007B0C(?);
// extern ? sub_08007B54(?);
// extern ? sub_08007BFC(?);
// extern ? sub_08007CA4(?);
// extern ? sub_08007CD4(?);
// extern ? sub_08007CE0(?);
// extern ? sub_08007DD4(?);
// extern ? sub_08007EB0(?);
// extern ? sub_08007F24(?);
// extern ? sub_08007F44(?);
// extern ? sub_08007FDC(?);
// extern ? sub_08007FE8(?);
// extern ? sub_0800801C(?);
// extern ? sub_08008070(?);
// extern ? sub_0800808C(?);
// extern ? sub_0800819C(?);
// extern ? sub_080081B0(?);
// extern ? sub_08008268(?);
// extern ? sub_080082C8(?);
// extern ? sub_08008328(?);
// extern ? sub_080083A8(?);
// extern ? sub_080083F0(?);
// extern ? sub_0800850C(?);
// extern ? sub_080085A0(?);
// extern ? sub_08008638(?);
// extern ? sub_080086F0(?);
// extern ? sub_0800876C(?);
// extern ? sub_080087CC(?);
// extern ? sub_080087F8(?);
// extern ? sub_080088B0(?);
// extern ? sub_08008968(?);
// extern ? sub_080089D4(?);
// extern ? sub_08008A40(?);
// extern ? sub_08008AF0(?);
// extern ? sub_08008B08(?);
// extern ? sub_08008B4C(?);
// extern ? sub_08008B98(?);
// extern ? sub_08008C1C(?);
// extern ? sub_08008CA8(?);
// extern ? sub_08008CC0(?);
// extern ? sub_08008CE4(?);
// extern ? sub_08008D0C(?);
// extern ? sub_08008D70(?);
// extern ? sub_08009018(?);
// extern ? sub_080090A8(?);
// extern ? sub_08009470(?);
// extern ? sub_080094B8(?);
// extern ? sub_08009518(?);
// extern ? sub_08009604(?);
// extern ? sub_08009678(?);
// extern ? sub_08009690(?);
// extern ? sub_080096C0(?);
// extern ? sub_080096FC(?);
// extern ? sub_08009808(?);
// extern ? sub_0800981C(?);
// extern ? sub_08009834(?);
// extern ? sub_08009854(?);
// extern ? sub_08009884(?);
// extern ? sub_080098D0(?);
// extern ? sub_08009938(?);
// extern ? sub_080099AC(?);
// extern ? sub_08009AB8(?);
// extern ? sub_08009B3C(?);
// extern ? sub_08009B68(?);
// extern ? sub_08009B94(?);
// extern ? sub_08009BD0(?);
// extern ? sub_08009BDC(?);
// extern ? sub_08009BE8(?);
// extern ? sub_08009BF4(?);
// extern ? sub_08009C04(?);
// extern ? sub_08009C14(?);
// extern ? sub_08009C20(?);
// extern ? sub_08009C2C(?);
// extern ? sub_08009C38(?);
// extern ? sub_08009C44(?);
// extern ? sub_08009C50(?);
// extern ? sub_08009C7C(?);
// extern ? sub_08009CA4(?);
// extern ? sub_08009CB0(?);
// extern ? sub_08009CBC(?);
// extern ? sub_08009CC8(?);
// extern ? sub_08009CD4(?);
// extern ? sub_08009D14(?);
// extern ? sub_08009D28(?);
// extern ? sub_08009D4C(?);
// extern ? sub_08009D6C(?);
// extern ? sub_08009D94(?);
// extern ? sub_08009DB4(?);
// extern ? sub_08009E44(?);
// extern ? sub_08009E64(?);
// extern ? sub_08009EA8(?);
// extern ? sub_08009EDC(?);
// extern ? sub_08009F18(?);
// extern ? sub_08009F50(?);
// extern ? sub_08009F78(?);
// extern ? sub_0800A094(?);
// extern ? sub_0800A0B8(?);
// extern ? sub_0800A0DC(?);
// extern ? sub_0800A0F0(?);
// extern ? sub_0800A110(?);
// extern ? sub_0800A124(?);
// extern ? sub_0800A160(?);
// extern ? sub_0800A1A4(?);
// extern ? sub_0800A1EC(?);
// extern ? sub_0800A240(?);
// extern ? sub_0800A264(?);
// extern ? sub_0800A2CC(?);
// extern ? sub_0800A358(?);
// extern ? sub_0800A37C(?);
// extern ? sub_0800A3BC(?);

// asm/code_0800A3F0.s

// extern ? sub_0800A410(?);
// extern ? sub_0800A444(?);
// extern ? sub_0800A474(?);
// extern ? sub_0800A4A8(?);
// extern ? sub_0800A4EC(?);
// extern ? sub_0800A520(?);
// extern ? sub_0800A554(?);
// extern ? sub_0800A5B4(?);
// extern ? sub_0800A608(?);
// extern ? sub_0800A630(?);
// extern ? sub_0800A664(?);
// extern ? sub_0800A694(?);
// extern ? sub_0800A734(?);
// extern ? sub_0800A778(?);
// extern ? sub_0800A7B8(?);
// extern ? sub_0800AC68(?);
// extern ? sub_0800AC88(?);
// extern ? sub_0800AC9C(?);
// extern ? sub_0800ACB0(?);
// extern ? sub_0800ACD0(?);
// extern ? sub_0800AD04(?);
// extern ? sub_0800AD28(?);
// extern ? sub_0800AD4C(?);
// extern ? sub_0800AD80(?);
// extern ? sub_0800ADA8(?);
// extern ? sub_0800ADD8(?);
// extern ? sub_0800AF0C(?);
// extern ? sub_0800AFA8(?);
// extern ? sub_0800B024(?);
// extern ? sub_0800B068(?);
// extern ? sub_0800B0B0(?);
// extern ? sub_0800B0F8(?);
// extern ? sub_0800B15C(?);
// extern ? sub_0800B200(?);
// extern ? sub_0800B354(?);
// extern ? sub_0800B3A4(?);
// extern ? sub_0800B3E4(?);
// extern ? sub_0800B74C(?);
// extern ? sub_0800B764(?);
// extern ? sub_0800B788(?);
// extern ? sub_0800B7AC(?);
// extern ? sub_0800BABC(?);
// extern ? sub_0800BB28(?);
// extern ? sub_0800BB60(?);
// extern ? sub_0800BBF0(?);
// extern ? sub_0800BC30(?);
// extern ? sub_0800BD70(?);
// extern ? sub_0800BD94(?);
// extern ? sub_0800BDAC(?);
// extern ? sub_0800BDD0(?);
// extern ? sub_0800BE8C(?);
// extern ? sub_0800BEAC(?);
// extern ? sub_0800BF04(?);
// extern ? sub_0800BF38(?);
// extern ? sub_0800BFB0(?);
// extern ? sub_0800C004(?);
// extern ? sub_0800C01C(?);
// extern ? sub_0800C058(?);
// extern ? sub_0800C06C(?);
// extern ? sub_0800C224(?);
// extern ? sub_0800C424(?);
// extern ? sub_0800C49C(?);
// extern ? sub_0800C51C(?);
// extern ? sub_0800C56C(?);
// extern ? sub_0800C584(?);
// extern ? sub_0800C5C0(?);
// extern ? sub_0800C688(?);
// extern ? sub_0800C794(?);
// extern ? sub_0800CC94(?);
// extern ? sub_0800CCCC(?);
// extern ? sub_0800CCF0(?);
// extern ? sub_0800CD2C(?);
// extern ? sub_0800CD7C(?);
// extern ? sub_0800CDCC(?);
// extern ? sub_0800CE28(?);
// extern ? sub_0800CEB0(?);
// extern ? sub_0800CF34(?);
// extern ? sub_0800CFC0(?);
// extern ? sub_0800D000(?);
// extern ? sub_0800D02C(?);
// extern ? sub_0800D194(?);
// extern ? sub_0800D218(?);
// extern ? sub_0800D270(?);
// extern ? sub_0800D2A8(?);
// extern ? sub_0800D2E0(?);
// extern ? sub_0800D328(?);
// extern ? sub_0800D434(?);
// extern ? sub_0800D4C0(?);
// extern ? sub_0800D55C(?);
// extern ? sub_0800D81C(?);
// extern ? sub_0800D880(?);
// extern ? sub_0800D8A4(?);
// extern ? sub_0800D8E0(?);
// extern ? sub_0800DA70(?);
// extern ? sub_0800DA88(?);
// extern ? sub_0800DAC4(?);
// extern ? sub_0800DC80(?);
// extern ? sub_0800DCBC(?);
// extern ? sub_0800DDF4(?);
// extern ? sub_0800DE60(?);
// extern ? sub_0800DE74(?);
// extern ? sub_0800E044(?);
// extern ? sub_0800E084(?);
// extern ? sub_0800E0C0(?);
// extern ? sub_0800E1AC(?);
// extern ? sub_0800E224(?);
// extern ? sub_0800E248(?);
// extern ? sub_0800E274(?);
// extern ? sub_0800E294(?);
// extern ? sub_0800E2BC(?);
// extern ? sub_0800E2E4(?);
// extern ? sub_0800E314(?);
// extern ? sub_0800E388(?);
// extern ? sub_0800E3A0(?);
// extern ? sub_0800E3EC(?);
// extern ? sub_0800E420(?);
// extern ? sub_0800E53C(?);
// extern ? sub_0800E55C(?);
// extern ? sub_0800E6D4(?);
// extern ? sub_0800E6E8(?);
// extern ? sub_0800E724(?);
// extern ? sub_0800E750(?);
// extern ? sub_0800E7CC(?);
// extern ? sub_0800E7EC(?);
// extern ? sub_0800E8E0(?);
// extern ? sub_0800E8F4(?);
// extern ? sub_0800E930(?);
// extern ? sub_0800E95C(?);
// extern ? sub_0800E9B8(?);
// extern ? sub_0800E9D8(?);
// extern ? sub_0800EA50(?);
// extern ? sub_0800EA64(?);
// extern ? sub_0800EAA0(?);
// extern ? sub_0800EACC(?);
// extern ? sub_0800EAE4(?);
// extern ? sub_0800EB2C(?);
// extern ? sub_0800EC0C(?);
// extern ? sub_0800EC2C(?);
// extern ? sub_0800ED14(?);
// extern ? sub_0800ED3C(?);
// extern ? sub_0800ED9C(?);
// extern ? sub_0800EE70(?);
// extern ? sub_0800EE8C(?);
// extern ? sub_0800EEA0(?);
// extern ? sub_0800EED4(?);
// extern ? sub_0800EF74(?);
// extern ? sub_0800EFB4(?);
// extern ? sub_0800EFF4(?);
// extern ? sub_0800F008(?);
// extern ? sub_0800F0C0(?);
// extern ? sub_0800F0CC(?);
// extern ? sub_0800F11C(?);
// extern ? sub_0800F16C(?);
// extern ? sub_0800F1BC(?);
// extern ? sub_0800F1E4(?);
// extern ? sub_0800F220(?);
// extern ? sub_0800F254(?);
// extern ? sub_0800F29C(?);
// extern ? sub_0800F2CC(?);
// extern ? sub_0800F2F8(?);
// extern ? sub_0800F324(?);
// extern ? sub_0800F3A0(?);
// extern ? sub_0800F3CC(?);
// extern ? sub_0800F3EC(?);
// extern ? sub_0800F43C(?);
// extern ? sub_0800F4BC(?);
// extern ? sub_0800F504(?);
// extern ? sub_0800FA5C(?);
// extern ? sub_0800FA6C(?);
// extern ? sub_0800FBFC(?);
// extern ? sub_0800FC0C(?);
// extern ? sub_0800FC38(?);
// extern ? sub_0800FC64(?);
// extern ? sub_0800FCA4(?);
// extern ? sub_0800FCB0(?);
// extern ? sub_0800FCCC(?);
// extern ? sub_0800FCEC(?);
// extern ? sub_0800FD0C(?);
// extern ? sub_0800FD50(?);
// extern ? sub_0800FDB0(?);
// extern ? sub_0800FE04(?);
// extern ? sub_0800FE5C(?);
// extern ? sub_0800FE68(?);
// extern ? sub_0800FEC0(?);
// extern ? sub_0800FED0(?);
// extern ? sub_0800FEEC(?);
// extern ? sub_0800FF44(?);
// extern ? sub_0800FF9C(?);
// extern ? sub_080100E4(?);
// extern ? sub_08010118(?);
// extern ? sub_08010120(?);
// extern ? sub_08010154(?);
// extern ? sub_0801015C(?);
// extern ? sub_08010170(?);
// extern ? sub_08010180(?);
// extern ? sub_08010190(?);
// extern ? sub_0801019C(?);
// extern ? sub_080101C0(?);
// extern ? sub_08010204(?);
// extern ? sub_08010430(?);
// extern ? sub_08010470(?);
// extern ? sub_08010490(?);
// extern ? sub_080104F0(?);
// extern ? sub_0801051C(?);
// extern ? sub_08010580(?);

// extern ? sub_08010738(?);
// extern ? sub_08010764(?);
// extern ? sub_08010770(?);
// extern ? sub_08010790(?);
// extern ? sub_080107AC(?);
// extern ? sub_080107C4(?);
// extern ? SceEeprom_GetSubFileAdr(?);
// extern ? SceEeprom_GetSubFileSize(?);
// extern ? sub_080108A4(?);
// extern ? sub_080108BC(?);

// extern ? sub_080108D8(?);
// extern ? sub_08010900(?);
// extern ? sub_08010910(?);
// extern ? sub_0801097C(?);
// extern ? sub_080109C0(?);
// extern ? sub_080109F0(?);
// extern ? sub_08010A48(?);
// extern ? sub_08010A78(?);
extern void sub_08010AF0(u32 arg0, u32 arg1);
// extern ? sub_08010B34(?);
// extern ? sub_08010B8C(?);
// extern ? sub_08010C14(?);
// extern ? sub_08010C64(?);
// extern ? sub_08010CBC(?);
// extern ? sub_08010CF0(?);
// extern ? sub_08010D08(?);
// extern ? sub_08010D20(?);
// extern ? sub_08010D34(?);
// extern ? sub_08010D60(?);
// extern ? sub_08010D88(?);
// extern ? sub_08010DEC(?);
// extern ? sub_08010E64(?);
// extern ? sub_08010E94(?);
// extern ? sub_08010ED8(?);
// extern ? sub_08010F30(?);
// extern ? sub_08010F64(?);
// extern ? sub_08010FD8(?);
// extern ? sub_08010FE4(?);
// extern ? sub_08011058(?);
// extern ? sub_080110DC(?);
extern void *sub_08011124(u32 file);
// extern ? sub_080112F4(?);
// extern ? sub_08011328(?);
// extern ? sub_08011354(?);
// extern ? sub_08011380(?);
// extern ? sub_08011414(?);
// extern ? sub_08011438(?);
// extern ? sub_080114F4(?);
// extern ? sub_080117CC(?);
// extern ? sub_0801181C(?);
// extern ? sub_0801186C(?);
// extern ? sub_08011880(?);
// extern ? sub_080118A0(?);
// extern ? sub_080118D4(?);
// extern ? sub_08011908(?);
// extern ? sub_0801194C(?);
// extern ? sub_0801197C(?);
// extern ? sub_080119A8(?);
// extern ? sub_080119D4(?);
// extern ? sub_08011A08(?);
// extern ? sub_08011A3C(?);
// extern ? sub_08011A54(?);
// extern ? sub_08011A78(?);
// extern ? sub_08011AD8(?);
// extern ? sub_08011B68(?);
// extern ? sub_08011C18(?);
// extern ? sub_08011C70(?);
// extern ? sub_08011C80(?);
// extern ? sub_08011C90(?);
// extern ? sub_08011CA4(?);
// extern ? sub_08011CD8(?);
// extern ? sub_08011CF4(?);
// extern ? sub_08011D10(?);
// extern ? sub_08011D8C(?);
// extern ? sub_08011E44(?);
// extern ? sub_08011E64(?);
// extern ? sub_08011E9C(?);
// extern ? sub_08011EB8(?);
// extern ? sub_08011EC6(?);
// extern ? sub_08013808(?);
// extern ? sub_08013818(?);
// extern ? sub_080138B8(?);
// extern ? sub_080138F0(?);
// extern ? sub_080139A4(?);
// extern ? sub_08013AD8(?);
// extern ? sub_08013F38(?);
// extern ? sub_08013F74(?);
// extern ? sub_08013FAC(?);
// extern ? sub_08013FC0(?);
// extern ? sub_08013FF0(?);
// extern ? sub_08014030(?);
// extern ? sub_08014068(?);
// extern ? sub_08014088(?);
// extern ? sub_08014098(?);
// extern ? sub_080140AC(?);
// extern ? sub_08014220(?);
// extern ? sub_080142D0(?);
// extern ? sub_080142F0(?);
// extern ? sub_0801439C(?);
// extern ? sub_08014458(?);
// extern ? sub_0801458C(?);
// extern ? sub_080145FC(?);
// extern ? sub_080146AC(?);
// extern ? sub_0801477C(?);
// extern ? sub_080147AC(?);
// extern ? sub_08014834(?);
// extern ? sub_0801486C(?);
// extern ? sub_08014A20(?);
// extern ? sub_08014A60(?);
// extern ? sub_08014BB4(?);
// extern ? sub_08014C68(?);
// extern ? sub_08014D3C(?);
// extern ? sub_08014DF0(?);
// extern ? sub_08015088(?);
// extern ? sub_08015114(?);
// extern ? sub_08015188(?);
// extern ? sub_080152B8(?);
// extern ? sub_08015438(?);
// extern ? sub_080154B4(?);
// extern ? sub_080154D4(?);
// extern ? sub_080154E0(?);
// extern ? sub_080154EC(?);
// extern ? sub_080154F8(?);
// extern ? sub_08015504(?);
// extern ? sub_080155BC(?);
// extern ? sub_080155F8(?);
// extern ? sub_08015624(?);
// extern ? sub_08015688(?);
// extern ? sub_080156CC(?);
// extern ? sub_08015714(?);
// extern ? sub_0801576C(?);
// extern ? sub_08015778(?);
// extern ? sub_08015794(?);
// extern ? sub_08015818(?);
// extern ? sub_0801583C(?);
// extern ? sub_08015890(?);
// extern ? sub_08015928(?);
// extern ? sub_08015968(?);
// extern ? sub_080159A4(?);
// extern ? sub_080159C4(?);
// extern ? sub_08015A14(?);
// extern ? sub_08015A94(?);
// extern ? sub_08015B04(?);
// extern ? sub_08015B60(?);
// extern ? sub_08015B6C(?);
// extern ? sub_08015BA8(?);
// extern ? sub_08015BD0(?);
// extern ? sub_08015BF8(?);
// extern ? sub_08015C08(?);
// extern ? sub_08015C30(?);
// extern ? sub_08015C58(?);
// extern ? sub_08015C7C(?);
// extern ? sub_08016594(?);
// extern ? sub_08016870(?);
// extern ? sub_08016C1C(?);
// extern ? sub_08016D14(?);
// extern ? sub_08016D4C(?);
// extern ? sub_08016D88(?);
// extern ? sub_08016E18(?);
// extern ? sub_08016E2C(?);
// extern ? sub_08016E40(?);
// extern ? sub_08016E54(?);
// extern ? sub_08016E98(?);
// extern ? sub_08016EB0(?);
// extern ? sub_08016EC8(?);
// extern ? sub_08016EFC(?);
// extern ? sub_08016F40(?);
// extern ? sub_08016FB0(?);
// extern ? sub_08017014(?);
// extern ? sub_08017040(?);
// extern ? sub_0801705C(?);
// extern ? sub_08017088(?);
// extern ? sub_080170CC(?);
// extern ? sub_080170E4(?);
// extern ? sub_08017118(?);
// extern ? sub_0801714C(?);
// extern ? sub_0801718C(?);
// extern ? sub_080171D8(?);
// extern ? sub_08017228(?);
// extern ? sub_08017288(?);
// extern ? sub_080172B4(?);
// extern ? sub_080173DC(?);
// extern ? sub_080173F0(?);
// extern ? sub_08017440(?);
// extern ? sub_08017464(?);
// extern ? sub_0801749C(?);
// extern ? sub_080174C4(?);
// extern ? sub_08017520(?);
// extern ? sub_080175C4(?);
// extern ? sub_08017650(?);
// extern ? sub_08017678(?);
// extern ? sub_08017720(?);
// extern ? sub_080177B8(?);
// extern ? sub_08017828(?);
// extern ? sub_08017868(?);
// extern ? sub_080178D8(?);
// extern ? sub_08017950(?);
// extern ? sub_08017B98(?);
// extern ? sub_08017C0C(?);
// extern ? sub_08017C1C(?);
// extern ? sub_08017C64(?);
// extern ? sub_08017C8C(?);
// extern ? sub_08017CCC(?);
// extern ? sub_08017D1C(?);
// extern ? sub_08017D88(?);
// extern ? sub_08017E04(?);
// extern ? sub_08017E34(?);
// extern ? sub_08017E60(?);
// extern ? sub_08017ECC(?);
// extern ? sub_08018058(?);
// extern ? sub_0801809C(?);
// extern ? sub_080180D0(?);
// extern ? sub_08018104(?);
// extern ? sub_080182F0(?);
// extern ? sub_0801857C(?);
// extern ? sub_080185D4(?);
// extern ? sub_08018730(?);
// extern ? sub_08018748(?);
// extern ? sub_08018760(?);
// extern ? sub_080187C8(?);
// extern ? sub_080187F0(?);
// extern ? sub_080188A8(?);
// extern ? sub_080188CC(?);
// extern ? sub_080188DC(?);
// extern ? sub_080189B4(?);
// extern ? sub_08018AB4(?);
// extern ? sub_08018AF4(?);
// extern ? sub_08018B18(?);
// extern ? sub_08018B40(?);
// extern ? sub_08018B60(?);
// extern ? sub_08018B70(?);
// extern ? sub_08018B90(?);
// extern ? sub_08018C6C(?);
// extern ? sub_08018CA0(?);
// extern ? sub_08018CDC(?);
// extern ? sub_08018D30(?);
// extern ? sub_08018EA4(?);
// extern ? sub_08018F28(?);
// extern ? sub_08018FB4(?);
// extern ? sub_08018FDC(?);
// extern ? sub_08018FFC(?);
// extern ? sub_0801901C(?);
// extern ? sub_08019024(?);
// extern ? sub_0801902C(?);
// extern ? sub_08019038(?);
// extern ? sub_0801903C(?);
// extern ? sub_08019054(?);
// extern ? sub_08019068(?);
// extern ? sub_08019070(?);
// extern ? sub_080190B0(?);
// extern ? sub_08019160(?);
// extern ? sub_08019180(?);
// extern ? sub_08019190(?);
// extern ? sub_080191D8(?);
// extern ? sub_08019274(?);
// extern ? sub_080192A4(?);
// extern ? sub_08019304(?);
// extern ? sub_080193F4(?);
// extern ? sub_08019430(?);
// extern ? sub_0801948C(?);
// extern ? sub_080194EC(?);
// extern ? sub_08019544(?);
// extern ? sub_080195C8(?);
// extern ? sub_08019688(?);
// extern ? sub_080196D0(?);
// extern ? sub_08019764(?);
// extern ? sub_08019788(?);
// extern ? sub_080197AC(?);
// extern ? sub_08019840(?);
// extern ? sub_08019878(?);
// extern ? sub_08019888(?);
// extern ? sub_0801993C(?);
// extern ? sub_08019ABC(?);
// extern ? sub_08019B9C(?);
// extern ? sub_08019BC8(?);
// extern ? sub_08019BE0(?);
// extern ? sub_08019C24(?);
// extern ? sub_08019C48(?);
// extern ? sub_08019D08(?);
// extern ? sub_0801A24C(?);
// extern ? sub_0801A274(?);
// extern ? sub_0801A29C(?);
// extern ? sub_0801A2DC(?);
// extern ? sub_0801A330(?);
// extern ? sub_0801A4A0(?);
// extern ? sub_0801A544(?);
// extern ? sub_0801A82C(?);
// extern ? sub_0801A924(?);
// extern ? sub_0801A990(?);
// extern ? sub_0801A9FC(?);
// extern ? sub_0801AA00(?);
// extern ? sub_0801AD6C(?);
// extern ? sub_0801AD94(?);
// extern ? sub_0801ADBC(?);
// extern ? sub_0801B030(?);
// extern ? sub_0801B114(?);
// extern ? sub_0801B188(?);
// extern ? sub_0801B18C(?);
// extern ? sub_0801B3FC(?);
// extern ? sub_0801B424(?);
// extern ? sub_0801B44C(?);
// extern ? sub_0801B5F0(?);
// extern ? sub_0801B6CC(?);
// extern ? sub_0801B734(?);
// extern ? sub_0801B744(?);
// extern ? sub_0801B750(?);
// extern ? sub_0801B774(?);
// extern ? sub_0801B794(?);
// extern ? sub_0801B7A0(?);
// extern ? sub_0801B7AC(?);
// extern ? sub_0801B7B8(?);
// extern ? sub_0801B7C4(?);
// extern ? sub_0801B87C(?);
// extern ? sub_0801B8E8(?);
// extern ? sub_0801B8F8(?);
// extern ? sub_0801B934(?);
// extern ? sub_0801B94C(?);
// extern ? sub_0801B9D4(?);
// extern ? sub_0801BA14(?);
// extern ? sub_0801BA4C(?);
// extern ? sub_0801BA84(?);
// extern ? sub_0801BAD0(?);
// extern ? sub_0801BAF8(?);
// extern ? sub_0801BB20(?);
// extern ? sub_0801BB3C(?);
// extern ? sub_0801BCF8(?);
// extern ? sub_0801BF0C(?);
// extern ? sub_0801BF74(?);
// extern ? sub_0801BFE0(?);
// extern ? sub_0801C00C(?);
// extern ? sub_0801C0C0(?);
// extern ? sub_0801C110(?);
// extern ? sub_0801C144(?);
// extern ? sub_0801C198(?);
// extern ? sub_0801C274(?);
// extern ? sub_0801C2C0(?);
// extern ? sub_0801C2F0(?);
// extern ? sub_0801C324(?);
// extern ? sub_0801C3C8(?);
// extern ? sub_0801C3E8(?);
// extern ? sub_0801C400(?);
// extern ? sub_0801C430(?);
// extern ? sub_0801C440(?);
// extern ? sub_0801C4D0(?);
// extern ? sub_0801C4F8(?);
// extern ? sub_0801C524(?);
// extern ? sub_0801C574(?);
// extern ? sub_0801C634(?);
// extern ? sub_0801C91C(?);
// extern ? sub_0801C940(?);
// extern ? sub_0801C9A8(?);
// extern ? sub_0801CA1C(?);
// extern ? sub_0801CAC0(?);
// extern ? sub_0801CBA0(?);
// extern ? sub_0801CC00(?);
// extern ? sub_0801CC64(?);
// extern ? sub_0801CCC4(?);
// extern ? sub_0801CEEC(?);
// extern ? sub_0801CFC4(?);
// extern ? sub_0801D030(?);
// extern ? sub_0801D09C(?);
// extern ? sub_0801D158(?);
// extern ? sub_0801D200(?);
// extern ? sub_0801D27C(?);
// extern ? sub_0801D324(?);
// extern ? sub_0801D48C(?);
// extern ? sub_0801D5D4(?);
// extern ? sub_0801D5F0(?);
// extern ? sub_0801D640(?);
// extern ? sub_0801D65C(?);
// extern ? sub_0801D69C(?);
// extern ? sub_0801D6C8(?);
// extern ? sub_0801D8CC(?);
// extern ? sub_0801D8F0(?);
// extern ? sub_0801D8F4(?);
// extern ? sub_0801D968(?);
// extern ? sub_0801DAA0(?);
// extern ? sub_0801DB20(?);
// extern ? sub_0801DD7C(?);
// extern ? sub_0801DDD4(?);
// extern ? sub_0801E1C4(?);
// extern ? sub_0801E5A4(?);
// extern ? sub_0801E82C(?);
// extern ? sub_0801E9B0(?);
// extern ? sub_0801EA90(?);
// extern ? sub_0801EAD4(?);
// extern ? sub_0801EB6C(?);
// extern ? sub_0801EE48(?);
// extern ? sub_0801F30C(?);
// extern ? sub_0801F7C0(?);
// extern ? sub_0801FCD0(?);
// extern ? sub_0801FD14(?);
// extern ? sub_080218A6(?);
// extern ? sub_0802268A(?);
// extern ? sub_080226C0(?);
// extern ? sub_0802271A(?);
// extern ? sub_080233C8(?);
// extern ? sub_080233F8(?);
// extern ? sub_08023452(?);
// extern ? sub_0802497E(?);
// extern ? sub_0802498A(?);
// extern ? sub_080249BC(?);
// extern ? sub_08025624(?);
// extern ? sub_08025658(?);
// extern ? sub_080256B8(?);
// extern ? sub_08026206(?);
// extern ? sub_0802623C(?);
// extern ? sub_08026298(?);
// extern ? sub_08026BF6(?);
// extern ? sub_08026C28(?);
// extern ? sub_08027458(?);
// extern ? sub_08027FFE(?);
// extern ? sub_08028030(?);
// extern ? sub_0802816C(?);
// extern ? sub_0802846C(?);
// extern ? sub_08028478(?);
// extern ? sub_08028484(?);
// extern ? sub_080284A0(?);
// extern ? sub_080284AC(?);
// extern ? sub_080284B8(?);
// extern ? sub_080284C4(?);
// extern ? sub_080284D0(?);
// extern ? sub_08028928(?);
// extern ? sub_08028954(?);
// extern ? sub_080289F8(?);
// extern ? sub_08029004(?);
// extern ? sub_08029558(?);
// extern ? sub_0802A35C(?);
// extern ? sub_0802B058(?);
// extern ? sub_0802BF0A(?);
// extern ? sub_0802BF1E(?);
// extern ? sub_0802BF34(?);
// extern ? sub_0802C022(?);
// extern ? sub_0802CD1C(?);
// extern ? sub_0802CDC2(?);
// extern ? sub_0802CE7E(?);
// extern ? sub_0802CE9C(?);
// extern ? sub_0802CEFE(?);
// extern ? sub_0802DECE(?);
// extern ? sub_0802DEF2(?);
// extern ? sub_0802DF68(?);
// extern ? sub_0802DF8C(?);
// extern ? sub_0802DFF4(?);
// extern ? sub_0802EDA0(?);
// extern ? sub_0802F34A(?);
// extern ? sub_0802F34C(?);
// extern ? sub_0802F392(?);
// extern ? sub_0802FA0A(?);
// extern ? sub_0802FA2C(?);
// extern ? sub_0802FA86(?);
// extern ? sub_080307BC(?);
// extern ? sub_08030C58(?);
// extern ? sub_08030C78(?);
// extern ? sub_08030CD6(?);
// extern ? sub_08030D00(?);
// extern ? sub_08030E20(?);
// extern ? sub_08031190(?);
// extern ? sub_08031280(?);
// extern ? sub_080312CC(?);
// extern ? sub_0803137C(?);
// extern ? sub_08031598(?);
// extern ? sub_08031978(?);
// extern ? sub_08031BE0(?);
// extern ? sub_08032008(?);
// extern ? sub_080322E4(?);
// extern ? sub_080322E8(?);
// extern ? sub_0803232C(?);
// extern ? sub_080324C4(?);
// extern ? sub_08032880(?);
// extern ? sub_08032C5C(?);
// extern ? sub_08032D78(?);
// extern ? sub_08033080(?);
// extern ? sub_080333A0(?);
// extern ? sub_080333CC(?);
// extern ? sub_08033650(?);
// extern ? sub_080339F4(?);
// extern ? sub_08033CC0(?);
// extern ? sub_08033E94(?);
// extern ? sub_08033F0C(?);
// extern ? sub_08034188(?);
// extern ? sub_08034D20(?);
// extern ? sub_08034D34(?);
// extern ? sub_080351D4(?);
// extern ? sub_080355A8(?);
// extern ? sub_08035668(?);
// extern ? sub_0803572C(?);
// extern ? sub_080357A0(?);
// extern ? sub_08035858(?);
// extern ? sub_08035A18(?);
// extern ? sub_08035E30(?);
// extern ? sub_08035E60(?);
// extern ? sub_08035ECC(?);
// extern ? sub_08035F34(?);
// extern ? sub_08036238(?);
// extern ? sub_08036330(?);
// extern ? sub_08036344(?);
// extern ? sub_0803638C(?);
// extern ? sub_080363F4(?);
// extern ? sub_08036470(?);
// extern ? sub_080364BC(?);
// extern ? sub_0803655C(?);
// extern ? sub_08036EF4(?);
// extern ? sub_080371D8(?);
// extern ? sub_080372AC(?);
// extern ? sub_080374C0(?);
// extern ? sub_08037524(?);
// extern ? sub_0803755C(?);
// extern ? sub_08037594(?);
// extern ? sub_080375AC(?);
// extern ? sub_080375FC(?);
// extern ? sub_08037608(?);
// extern ? sub_08037614(?);
// extern ? sub_08037668(?);
// extern ? sub_08037674(?);
// extern ? sub_08037680(?);
// extern ? sub_080376D0(?);
// extern ? sub_080376DC(?);
// extern ? sub_080376E8(?);
// extern ? sub_08037738(?);
// extern ? sub_08037744(?);
// extern ? sub_08037750(?);
// extern ? sub_080377AC(?);
// extern ? sub_080377BC(?);
// extern ? sub_080377C8(?);
// extern ? sub_080377D4(?);
// extern ? sub_080377E0(?);
// extern ? sub_080377EC(?);
// extern ? sub_080377F8(?);
// extern ? sub_08037810(?);
// extern ? sub_08037960(?);
// extern ? sub_0803796C(?);
// extern ? sub_08037978(?);
// extern ? sub_080379EA(?);
// extern ? sub_080382F4(?);
// extern ? sub_080388F0(?);
// extern ? sub_08038910(?);
// extern ? sub_08038924(?);
// extern ? sub_08038B90(?);
// extern ? sub_08038BAC(?);
// extern ? sub_08038C70(?);
// extern ? sub_08038D60(?);
// extern ? sub_08038E14(?);
// extern ? sub_08039250(?);
// extern ? sub_0803927C(?);
// extern ? sub_080392B4(?);
// extern ? sub_080392EC(?);
// extern ? sub_0803930C(?);
// extern ? sub_08039560(?);
// extern ? sub_08039658(?);
// extern ? sub_08039AA0(?);
// extern ? sub_0803A120(?);
// extern ? sub_0803A144(?);
// extern ? sub_0803A1A4(?);
// extern ? sub_0803A204(?);
// extern ? sub_0803A2C8(?);
// extern ? sub_0803A388(?);
// extern ? sub_0803A3C4(?);
// extern ? sub_0803A41C(?);
// extern ? sub_0803AA2C(?);
// extern ? sub_0803ACA4(?);
// extern ? sub_0803AE2C(?);
// extern ? sub_0803AE60(?);
// extern ? sub_0803AE70(?);
// extern ? sub_0803AE90(?);
// extern ? sub_0803AEC0(?);
// extern ? sub_0803AEDC(?);
// extern ? sub_0803AF50(?);
// extern ? sub_0803B020(?);
// extern ? sub_0803B054(?);
// extern ? sub_0803B07C(?);
// extern ? sub_0803B094(?);
// extern ? sub_0803B148(?);
// extern ? sub_0803B1C0(?);
// extern ? sub_0803B394(?);
// extern ? sub_0803B41C(?);
// extern ? sub_0803B67C(?);
// extern ? sub_0803B824(?);
// extern ? sub_0803B930(?);
// extern ? sub_0803BA84(?);
// extern ? sub_0803BC18(?);
// extern ? sub_0803BD8C(?);
// extern ? sub_0803BEBC(?);
// extern ? sub_0803C134(?);
// extern ? sub_0803C2B0(?);
// extern ? sub_0803C418(?);
// extern ? sub_0803C580(?);
// extern ? sub_0803C614(?);
// extern ? sub_0803C6F4(?);
// extern ? sub_0803C7D0(?);
// extern ? sub_0803C958(?);
// extern ? sub_0803CA00(?);
// extern ? sub_0803CA94(?);
// extern ? sub_0803CC90(?);
// extern ? sub_0803CD20(?);
// extern ? sub_0803CDAC(?);
// extern ? sub_0803CEAC(?);
// extern ? sub_0803D28C(?);
// extern ? sub_0803D2B0(?);
// extern ? sub_0803D2D8(?);
extern void sub_0803D308(void);
// extern ? sub_0803D31C(?);
// extern ? sub_0803D320(?);
// extern ? sub_0803D328(?);

// asm/AgbEeprom.s

extern u16 IdentifyEeprom(u16 eeprom_KbitSize);
// extern u16 SetEepromTimerIntr(u8 timerNo, void (**IntrFunc)(void));
extern u16 SetEepromTimerIntr(u8 timerNo, u32 *arg1);
extern void Dma3Transmit(u16 *src, u16 *dst, u16 trmCount);
extern u16 ReadEepromDword(u16 epAdr, u16 *dst);
extern u16 ProgramEepromDword(u16 epAdr, u16 *src);
extern u16 VerifyEepromDword(u16 epAdr, u16 *src);
extern u16 ProgramEepromDwordEx(u16 epAdr, u16 *src);

#endif // GUARD_FUNCTIONS_H
